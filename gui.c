            ...
            if (strcmp(memory[i].name, "PCB_pid") == 0) {
                ...
                for (int j = i; j < i + 6; j++) {
                    if (strcmp(memory[j].name, "PCB_state") == 0) {
                        if (strcmp(memory[j].data, "NEW") == 0) state = NEW;
                        else if (strcmp(memory[j].data, "READY") == 0) state = READY;
                        else if (strcmp(memory[j].data, "RUNNING") == 0) state = RUNNING;
                        else if (strcmp(memory[j].data, "BLOCKED") == 0) state = BLOCKED;
                        else if (strcmp(memory[j].data, "TERMINATED") == 0) state = TERMINATED;
                    }
...
void update_gui(AppWidgets *w) {
    ...
    switch (currentAlgorithm) {
        case FCFS: algo_str = "First Come First Serve"; break;
        case RR: algo_str = "Round Robin"; break;
        case MLFQ: algo_str = "Multilevel Feedback Queue"; break;
    }
